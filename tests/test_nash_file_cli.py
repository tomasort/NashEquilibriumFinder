...
from nash_file import analyze, best_response, cli, examples, payoffs, validate
...
"""
    ...
    def test_analyze_with_save_json(self, prisoners_dilemma_file):
        ...
        try:
            result = runner.invoke(analyze, [prisoners_dilemma_file, "--save-json", tmp_path])
            assert result.exit_code == 0
...
"""
    ...
    def test_payoffs_valid_strategies(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(payoffs, [test_game_file, "--p1-strategy", "0.7,0.3", "--p2-strategy", "0.4,0.6"])
        assert result.exit_code == 0
    ...
    def test_payoffs_invalid_probabilities_sum(self, test_game_file):
        ...
        result = runner.invoke(
            payoffs, [test_game_file, "--p1-strategy", "0.7,0.4", "--p2-strategy", "0.4,0.6"]  # sums to 1.1
        )
        assert result.exit_code != 0
    ...
    def test_payoffs_negative_probabilities(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(payoffs, [test_game_file, "--p1-strategy", "-0.1,1.1", "--p2-strategy", "0.4,0.6"])
        assert result.exit_code != 0
    ...
    def test_payoffs_wrong_dimensions(self, test_game_file):
        ...
        result = runner.invoke(
            payoffs,
            [test_game_file, "--p1-strategy", "0.3,0.3,0.4", "--p2-strategy", "0.4,0.6"],  # 3 values for 2x2 game
        )
        assert result.exit_code != 0
    ...
    def test_payoffs_invalid_format(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(payoffs, [test_game_file, "--p1-strategy", "abc,def", "--p2-strategy", "0.4,0.6"])
        assert result.exit_code != 0
...
"""
    ...
    def test_best_response_player1(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(best_response, [test_game_file, "--player", "1", "--opponent-strategy", "0.4,0.6"])
        assert result.exit_code == 0
    ...
    def test_best_response_player2(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(best_response, [test_game_file, "--player", "2", "--opponent-strategy", "0.7,0.3"])
        assert result.exit_code == 0
    ...
    def test_best_response_missing_player(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(best_response, [test_game_file, "--opponent-strategy", "0.4,0.6"])
        assert result.exit_code != 0
        assert "Missing option" in result.output
    ...
    def test_best_response_invalid_player(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(best_response, [test_game_file, "--player", "3", "--opponent-strategy", "0.4,0.6"])
        assert result.exit_code != 0
    ...
    def test_best_response_invalid_strategy(self, test_game_file):
        ...
        runner = CliRunner()
        result = runner.invoke(
            best_response, [test_game_file, "--player", "1", "--opponent-strategy", "0.3,0.4"]  # doesn't sum to 1
        )
        assert result.exit_code != 0
        assert "must be valid probabilities summing to 1" in result.output
    ...
    def test_best_response_wrong_dimensions(self, test_game_file):
        ...
        result = runner.invoke(
            best_response,
            [test_game_file, "--player", "1", "--opponent-strategy", "0.3,0.3,0.4"],  # 3 values for 2x2 game
        )
        assert result.exit_code != 0
