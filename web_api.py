...
def create_common_game():
    ...
    try:
        game_id, game = game_manager.create_common_game(
            ...
            **{k: v for k, v in data.items() if k != 'game_type'}
        )
...
def analyze_game(game_id):
    ...
    try:
        ...
        analysis = game_manager.analyze_game(
            game_id,
            find_nash=find_nash,
...
def calculate_expected_payoffs(game_id):
    ...
    try:
        eps = game_manager.calculate_expected_payoffs(
            game_id,
            p1_strategy=data['p1_strategy'],
            p2_strategy=data['p2_strategy']
        )
        
        return jsonify({
            'expected_payoffs': [float(eps[0]), float(eps[1])]
        })
...
def generate_random_beliefs(game_id):
    ...
    try:
        ...
        beliefs = game_manager.generate_random_beliefs(
            game_id,
            mode=mode
        )
        
        return jsonify({
            'beliefs': [beliefs[0], beliefs[1]]
        })
