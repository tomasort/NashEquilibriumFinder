...
from normal_form.game_file_parser import GameFileParser, GameFileParseError
from normal_form.game_manager import GameManager
from normal_form.utils import from_list_to_beliefs, get_coordinates_string, print_section_header, print_subsection_header
...
@click.option('--output', '-o', type=click.Choice(['normal', 'minimal', 'json']), 
              default='normal', help='Output format', show_default=True)
@click.option('--analyze-mixed/--no-mixed', default=True, 
              help='Whether to analyze mixed strategy equilibria for 2x2 games')
...
@cli.command()
@click.argument('game_file', type=click.Path(exists=True))
@click.option('--p1-strategy', type=str, required=True,
              help='Player 1 mixed strategy as comma-separated probabilities (e.g., 0.3,0.7)')
@click.option('--p2-strategy', type=str, required=True,
...
def payoffs(game_file, p1_strategy, p2_strategy):
    ...
    try:
        ...
        if len(p2_beliefs) != game.columns:
            click.echo(f"Error: Player 2 strategy has {len(p2_beliefs)} values but game has {game.columns} columns", err=True)
            sys.exit(1)
...
@cli.command()
@click.argument('game_file', type=click.Path(exists=True))
@click.option('--player', type=click.Choice(['1', '2']), required=True,
              help='Which player to analyze best responses for')
@click.option('--opponent-strategy', type=str, required=True,
              help='Opponent mixed strategy as comma-separated probabilities')
...
def examples():
    ...
    click.echo(f"  nash-file analyze examples/prisoners_dilemma.yml")
